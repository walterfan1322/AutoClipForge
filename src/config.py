...
def get_flux_api_base_url() -> str:
    """
    Gets the optional FLUX image service base URL.

    Returns:
        url (str): FLUX service base URL, or empty string if disabled.
    """
    with open(os.path.join(ROOT_DIR, "config.json"), "r") as file:
        return json.load(file).get("flux_api_base_url", "").strip()
